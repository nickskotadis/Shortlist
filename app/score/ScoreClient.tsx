...
function ScoreRing({ score, size = 80 }: { score: number; size?: number }) {
  ...
  const pct = score / 10;
  const color =
    score >= 8 ? "#35583F" : score >= 6 ? "#2F4A3C" : score >= 4 ? "#7A5C1E" : "#8F3A28";
...
function DimensionCard({ dimensionKey, score }: { dimensionKey: string; score: number }) {
  ...
  const color =
    score >= 8 ? "text-[var(--color-success)]" : score >= 6 ? "text-[var(--color-accent)]" : score >= 4 ? "text-[var(--color-warning)]" : "text-[var(--color-error)]";
  const bg =
    score >= 8 ? "bg-[var(--color-success-bg)] border-[var(--color-success-border)]" : score >= 6 ? "bg-[var(--color-accent-weak)] border-[var(--color-accent-weak-border)]" : score >= 4 ? "bg-[var(--color-warning-bg)] border-[var(--color-warning-border)]" : "bg-[var(--color-error-bg)] border-[var(--color-error-border)]";

  return (
    <div className={`rounded-md border p-5 ${bg}`}>
      ...
      <div className="h-1.5 bg-[var(--color-border)] rounded-full overflow-hidden mb-2">
        <div
          className={`h-full rounded-full transition-all duration-700 ${
            score >= 8 ? "bg-[var(--color-success)]" : score >= 6 ? "bg-[var(--color-accent)]" : score >= 4 ? "bg-[var(--color-warning)]" : "bg-[var(--color-error)]"
          }`}
...
export default function ScoreClient({
  ...
  const overallColor =
    !result ? ""
    : result.overall >= 8 ? "text-[var(--color-success)]"
    : result.overall >= 6 ? "text-[var(--color-accent)]"
    : result.overall >= 4 ? "text-[var(--color-warning)]"
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
      ...
      {/* Limit reached gate */}
      {limitReached && (
        <div className="bg-[var(--color-accent-weak)] border border-[var(--color-accent-weak-border)] rounded-md p-8 mb-6 text-center">
          <div className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-[var(--color-accent-weak)] border border-[var(--color-accent-weak-border)] mb-4">
          ...
          <p className="text-sm text-[var(--color-accent)] mb-6 max-w-sm mx-auto">
          ...
          <a
            href="/pricing"
            className="inline-flex items-center bg-[var(--color-accent)] hover:bg-[var(--color-accent-hover)] text-white text-sm font-semibold px-6 py-2.5 rounded-md shadow-sm transition-all hover:-translate-y-px"
          >
            Upgrade to Pro →
          </a>
        </div>
      ...
      {/* Input card + results */}
      {!limitReached && <>
      <div className="bg-[var(--color-surface)] rounded-md border border-[var(--color-border)] p-6 mb-6">
      ...
      {result && (
        <div className="space-y-6">
          {/* Overall score */}
          <div className="bg-[var(--color-surface)] rounded-md border border-[var(--color-border)] p-8 text-center">
            ...
            <ul className="space-y-3">
              {result.recommendations.map((rec, i) => (
                <li key={i} className="flex items-start gap-3">
                  <span className="shrink-0 w-5 h-5 rounded-full bg-[var(--color-accent-weak)] border border-[var(--color-accent-weak-border)] text-[var(--color-accent)] text-xs font-bold flex items-center justify-center mt-0.5">
                    {i + 1}
                  </span>
          ...
          {/* CTA */}
          <div className="bg-[var(--color-accent-weak)] border border-[var(--color-accent-weak-border)] rounded-md p-6 text-center">
